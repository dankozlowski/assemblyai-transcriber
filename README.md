...
This creates `Transcriber.xcodeproj` from `project.yml`. Re-run it after adding or
removing source files, or after editing `project.yml`. Edit `project.yml`, never
`Transcriber.xcodeproj` directly — changes made to the generated project are lost
on the next regeneration.

This step is required on a fresh clone: the `Transcriber` scheme lives in
`xcuserdata/`, which is gitignored, so `-scheme Transcriber` will not resolve
until the project has been generated at least once.
...
| `CODE_SIGN_IDENTITY` | `Transcriber Dev` | A stable signing identity is what stops macOS re-prompting for Screen Recording permission on every rebuild (see Setup step 3) |
